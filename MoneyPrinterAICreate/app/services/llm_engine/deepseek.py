...
class DeepSeekLLM(BaseLLM):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        Args:
            config: 模型配置参数，包含以下字段：
                ...
                - base_url: API请求地址，默认为"https://api.deepseek.com"
                - api_key: API密钥
                - temperature: 生成温度
                - max_tokens: 最大生成长度
                - timeout: 请求超时时间
        ...
        # 构建API路径
        self.chat_completions_url = self.base_url
    ...
    def generate(self, prompt: str, **kwargs) -> str:
        ...
        while True:
            try:
                ...
                print(f"Request URL: {self.chat_completions_url}, payload: {payload}, headers: {headers}")
                
                # 发送同步请求
                with httpx.Client(timeout=60000) as client:
    ...
    def _prepare_payload(self, prompt: str, stream: bool = False, **kwargs) -> Dict[str, Any]:
        """
        准备DeepSeek API请求体
        ...
        """
        # 合并默认参数和额外参数
        merged_kwargs = self._prepare_kwargs(**kwargs)
        
        # 构建基础请求体
        payload = {
            "model": merged_kwargs['model_name'],
            "messages": [
                {"role": "user", "content": prompt}
            ],
