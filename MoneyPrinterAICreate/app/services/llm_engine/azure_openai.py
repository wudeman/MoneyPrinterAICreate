...
class AzureOpenAILLM(BaseLLM):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        Args:
            config: 模型配置参数，包含以下字段：
                - model_name: 模型名称，如"gpt-4", "gpt-35-turbo"
                - base_url: Azure OpenAI API端点地址
                - api_key: API密钥
                - temperature: 生成温度
                - max_tokens: 最大生成长度
                - timeout: 请求超时时间
                - max_retries: 最大重试次数
                - api_version: Azure API版本，如"2023-05-15"
                - deployment_name: Azure部署名称（必需）
        """
        ...
        if self.deployment_name:
            # 如果提供了deployment_name，则构建完整URL
            if self.base_url:
                self.chat_completions_url = f"{self.base_url.rstrip('/')}/openai/deployments/{self.deployment_name}/chat/completions?api-version={self.api_version}"
            else:
                # 使用默认的Azure OpenAI端点
                self.chat_completions_url = f"https://default-resource.openai.azure.com/openai/deployments/{self.deployment_name}/chat/completions?api-version={self.api_version}"
        else:
            # 如果没有deployment_name，设置为空字符串
    ...
    def generate(self, prompt: str, **kwargs) -> str:
        ...
        while True:
            try:
                # 创建请求头 - Azure OpenAI使用api-key作为授权头
                headers = {
                    "Content-Type": "application/json",
                    "api-key": request_kwargs['api_key']
                }
                ...
                with httpx.Client(timeout=request_kwargs['timeout']) as client:
                    ...
                    if data.get('choices') and len(data['choices']) > 0:
                        ...
                        logger.error("Azure OpenAI API returned empty response")
                        return ""
